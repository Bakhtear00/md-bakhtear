...
const PurchaseModule: React.FC = () => {
    ...
    try {
      if (editingId) {
        ...
        if (!formData.isCredit) {
          ...
          alert('ক্রয় রেকর্ড সংরক্ষিত এবং ক্যাশ থেকে বিয়োগ হয়েছে!');
        } else {
          alert('বাকিতে ক্রয় রেকর্ড সংরক্ষিত হয়েছে!');
        }
      }
  ...
  const filteredPurchases = useMemo(() => {
    ...
    return purchases.filter(p => p.date.split('T')[0] === filterDate);
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm('আপনি কি এই হিসাবটি মুছে ফেলতে নিশ্চিত?')) {
      try {
        ...
        alert('ক্রয় হিসাব সফলভাবে মোছা হয়েছে!');
        if (editingId === id) {
          setEditingId(null);
          setFormData(initialFormState);
        }
    ...
    <div className="space-y-6 animate-fade-in pb-12">
      <div className={`bg-white p-6 md:p-8 rounded-[2rem] shadow-xl border-2 ${editingId ? 'border-orange-500' : 'border-gray-100'} no-print`}>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div className="col-span-1">
            <label className="text-sm font-black text-gray-700 mb-1.5 block ml-1">পিস</label>
            <input type="text" inputMode="numeric" pattern="[0-9]*" value={formData.pieces} onChange={(e) => setFormData({ ...formData, pieces: e.target.value })} className="w-full px-4 py-4 rounded-xl bg-white border-2 border-gray-200 text-gray-900 font-bold text-lg outline-none" placeholder="০" required />
          </div>
          <div className="col-span-1">
            <label className="text-sm font-black text-gray-700 mb-1.5 block ml-1">ওজন (কেজি)</label>
            <input type="text" inputMode="decimal" value={formData.kg} onChange={(e) => setFormData({ ...formData, kg: e.target.value })} className="w-full px-4 py-4 rounded-xl bg-white border-2 border-gray-200 text-gray-900 font-bold text-lg outline-none" placeholder="০.০০" required />
          </div>
          <div className="col-span-1">
            <label className="text-sm font-black text-gray-700 mb-1.5 block ml-1">দর (রেট)</label>
            <input type="text" inputMode="decimal" value={formData.rate} onChange={(e) => setFormData({ ...formData, rate: e.target.value })} className="w-full px-4 py-4 rounded-xl bg-white border-2 border-gray-200 text-gray-900 font-bold text-lg outline-none" placeholder="০" required />
          </div>
                ...
                <tr key={p.id} className="text-base hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-6 text-center no-print">
                    <div className="flex items-center justify-center gap-3">
                      ...
                      <button onClick={() => handleDelete(p.id)} className="text-red-500 p-2 bg-red-50 rounded-lg hover:bg-red-100">
                        <Trash2 size={18} />
                      </button>
                    </div>
                  </td>
              ...
              {filteredPurchases.length === 0 && (
                <tr>
                  <td colSpan={7} className="px-6 py-20 text-center text-gray-400 font-bold">
                    {filterDate ? 'এই তারিখে কোনো তথ্য নেই' : 'এখনও কোনো ক্রয় রেকর্ড করা হয়নি।'}
                  </td>
                </tr>
              )}
            </tbody>
