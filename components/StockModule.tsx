...
const StockModule: React.FC = () => {
  const { stock, purchases, sales, resets, lotHistory } = useData();

  // Fix: Explicitly typing `curr` in the reduce function to resolve a TypeScript error where it was inferred as `unknown`.
  const totalStockPieces = Object.values(stock).reduce((acc, curr: { pieces: number }) => acc + (curr.pieces || 0), 0);
              ...
              <tbody className="divide-y divide-gray-100">
                {lotHistory.length > 0 ? lotHistory.map((lot: LotArchive) => (
                  <tr key={lot.id} className="hover:bg-gray-50/50">
                    ...
                    <td className="px-4 py-5 text-right text-emerald-600 font-bold">৳{new Intl.NumberFormat('bn-BD').format(lot.totalPurchase)}</td>
                    <td className="px-4 py-5 text-right text-blue-600 font-bold">৳{new Intl.NumberFormat('bn-BD').format(lot.totalSale)}</td>
