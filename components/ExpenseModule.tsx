...
import { EXPENSE_CATEGORIES, getLocalDateString } from '../constants';
  ...
  const handleDelete = async (id: string) => {
    if(window.confirm('আপনি কি এই হিসাবটি মুছে ফেলতে নিশ্চিত?')) {
      try {
        ...
        alert('খরচের হিসাব সফলভাবে মোছা হয়েছে!');
        if (editingId === id) {
          setEditingId(null);
          setFormData(initialFormState);
        }
      ...
      <div className={`bg-white p-6 md:p-8 rounded-[2rem] shadow-xl border-2 ${editingId ? 'border-orange-500' : 'border-gray-100'} no-print`}>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="space-y-1">
            <label className="text-xs font-black text-gray-500 uppercase ml-1">তারিখ</label>
          ...
          <div className="space-y-1">
            <label className="text-xs font-black text-gray-500 uppercase ml-1">ধরণ</label>
            <select value={formData.category} onChange={(e) => setFormData({ ...formData, category: e.target.value })} className="w-full px-4 py-4 rounded-xl bg-gray-50 border-2 border-gray-200 text-gray-900 font-bold text-lg outline-none focus:border-red-500">
              {EXPENSE_CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
            </select>
          </div>
          <div className="space-y-1">
            <label className="text-xs font-black text-gray-500 uppercase ml-1">টাকা</label>
            <input type="text" inputMode="decimal" value={formData.amount} onChange={(e) => setFormData({ ...formData, amount: e.target.value })} className="w-full px-4 py-4 rounded-xl bg-gray-50 border-2 border-gray-200 text-gray-900 font-black text-xl outline-none" placeholder="0" required />
          </div>
          <div className="space-y-1">
            <label className="text-xs font-black text-gray-500 uppercase ml-1">নোট</label>
            <input type="text" value={formData.note} onChange={(e) => setFormData({ ...formData, note: e.target.value })} className="w-full px-4 py-4 rounded-xl bg-gray-50 border-2 border-gray-200 text-gray-900 font-medium outline-none" placeholder="বিবরণ" />
          </div>
                ...
                <tr key={e.id} className="text-base hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-5 text-center no-print">
                    <div className="flex items-center justify-center gap-3">
                      ...
                      <button onClick={() => handleDelete(e.id)} className="text-red-500 p-2 bg-red-50 rounded-lg hover:bg-red-100">
                        <Trash2 size={18} />
                      </button>
                    </div>
                  </td>
              ...
              {filteredExpenses.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-6 py-20 text-center text-gray-400 font-bold">
                    {filterDate ? 'এই তারিখে কোনো খরচ নেই' : 'এখনও কোনো খরচ রেকর্ড করা হয়নি।'}
                  </td>
                </tr>
              )}
            </tbody>
